...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Inicializar listener de Revalidación de Calidad
    inicializarBotonRevalidar();
});
...
async function exportarPDF() {
    ...
    try {
        ...
        Swal.fire({
            ...
            text: 'Ocurrió un error al preparar el reporte completo.',
            icon: 'error',
...
// -------------------------------------------------------------
// REVALIDACIÓN DE CALIDAD (SWEETALERT2 INDUSTRIAL)
// -------------------------------------------------------------
function inicializarBotonRevalidar() {
    const btnRevalidar = document.getElementById('btnRevalidarCalidad');
    if (!btnRevalidar) return;

    btnRevalidar.addEventListener('click', async function () {
        const selectCelda = document.getElementById('filtro-celda');
        const nombreCelda = selectCelda && selectCelda.value ? selectCelda.options[selectCelda.selectedIndex]?.text : "Todas las Celdas";

        const confirmacion = await Swal.fire({
            title: '¿Revalidar Calidad?',
            html: `Se recalculará la telemetría contra las recetas activas.<br><small class="text-muted">Ámbito: <b>${nombreCelda}</b></small>`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#ffc107',
            cancelButtonColor: '#6c757d',
            confirmButtonText: '<span class="text-dark fw-bold"><i class="bi bi-arrow-repeat me-1"></i>Sí, revalidar</span>',
            cancelButtonText: 'Cancelar',
            background: '#1e222d',
            color: '#ffffff',
            customClass: {
                popup: 'border border-secondary shadow-lg rounded-3'
            }
        });

        if (!confirmacion.isConfirmed) return;

        const textoOriginal = btnRevalidar.innerHTML;
        btnRevalidar.disabled = true;
        btnRevalidar.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Procesando...';

        Swal.fire({
            title: 'Procesando Revalidación',
            html: 'Evaluando telemetría y ajustando tolerancias en base de datos...',
        ...
        const idMaquina = selectCelda && selectCelda.value ? parseInt(selectCelda.value) : null;
        ...
        try {
            const response = await fetch('/Home/RevalidarCalidad', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    IdentificadorId: isNaN(idMaquina) ? null : idMaquina,
            ...
            btnRevalidar.innerHTML = textoOriginal;
        }
    });
}
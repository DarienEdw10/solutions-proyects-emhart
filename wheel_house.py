#Este ya funciona la salida 3 perfecta con el plc va de 4 en 4, falla salida 2 con un desface
#  Corregir mañana y probar, pero antes hacer git para copia de versiones
import os
import time
import serial
import pandas as pd
...
class ProcesadorTramasIndustriales:
    ...
    def procesar_trama(self, valores):
        ...
        # 3. LECTURA Y NORMALIZACIÓN BINARIA PURA
        ...
        # Normalización binaria bit 15 (sin constantes fajas)
        cnt_trama = cnt_raw % 32768 if cnt_raw >= 32768 else cnt_raw
 
        # 4. FILTRADO DINÁMICO DE CONTINUIDAD RELATIVA (SECUENCIA 1:1)
        if salida_fisica in [3, 10]:
            salida_val = 3
            
            # Anti-duplicado en la misma ventana de tiempo
            if fecha_maquina == self.ultima_fecha_s3 and cnt_trama == self.ultimo_cnt_s3:
                return None
 
            # Control de incremento de producción real (acepta solo avances de 1 a 10 pernos)
            if self.ultimo_cnt_s3 > 0:
                diferencia = cnt_trama - self.ultimo_cnt_s3
                if diferencia <= 0 or diferencia > 10:
                    return None
 
            self.ultimo_cnt_s3 = cnt_trama
            ...
            # Anti-duplicado en la misma ventana de tiempo
            if fecha_maquina == self.ultima_fecha_s2 and cnt_trama == self.ultimo_cnt_s2:
                return None
 
            # Control de incremento para Salida 2 (descarta tramas desfasadas o ecos)
            if self.ultimo_cnt_s2 > 0:
                diferencia = cnt_trama - self.ultimo_cnt_s2
                if diferencia <= 0 or diferencia > 10:
                    return None
 
            self.ultimo_cnt_s2 = cnt_trama
        ...
        try:
            ...
            if ser.inWaiting() >= 200:
                ...
                if len(valores) >= 200:
                    ...
                    if cnt_init > 0:
                        self.historico_procesados.add((salida_init, cnt_init))
                    print(f"[SYNC] Estado base sincronizado. Aguardando disparos en tiempo real...")
        except Exception as e:
